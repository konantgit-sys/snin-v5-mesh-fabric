...
async def nostr_connect_and_auth(sk, pubhex):
    """Connect to Nostr relay and authenticate via NIP-42."""
    ...
    challenge = data[1]
    auth_evt = Event(public_key=pubhex, content="", created_at=int(time.time()),
        kind=22242, tags=[["challenge", challenge], ["relay", "ws://127.0.0.1:8198"]])
    sk.sign_event(auth_evt)
    
    await ws.send_str(json.dumps(["AUTH", {
        "id": auth_evt.id, "pubkey": auth_evt.public_key, "created_at": auth_evt.created_at,
        "kind": auth_evt.kind, "tags": auth_evt.tags, "content": auth_evt.content, "sig": auth_evt.signature
    }]))
    
    msg = await ws.receive(timeout=10)
    resp = json.loads(msg.data)
    if resp[0] == 'OK' and resp[2] == True:
        return ws, session
    else:
        await ws.close()
        await session.close()
...
async def sync_to_nostr(cache):
    ...
    ws, session = await nostr_connect_and_auth(sk, pubhex)
    if not ws:
        logger.error("Nostr AUTH failed")
        return 0
