// ═══════════════════════════════════════════════
// SNIN Client v4.0 — Premium Interactive Engine
// ═══════════════════════════════════════════════
...
let state = { tab: 'feed', aiOnly: true, stats: null, ws: null, eventCount: 0, authorCount: 0 };
...
function connectWS() {
  try {
    ...
    state.ws.onopen = () => updateStatus(true, true);
    state.ws.onclose = () => { updateStatus(false, true); setTimeout(connectWS, 5000); };
    state.ws.onerror = () => updateStatus(false, true);
  ...
  } catch(_) { updateStatus(false, true); }
}
...
function prependPost(event) {
  ...
  if (firstCard) {
    ...
    if (newCard) {
      ...
      newCard.style.animation = 'fadeSlideIn 0.3s cubic-bezier(0.16, 1, 0.3, 1)';
    }
  } else {
    container.innerHTML = html;
  }
  // Keep max 50 posts in DOM
  ...
  document.querySelectorAll('.nav-item').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      if (tabEl) {
        ...
        tabEl.offsetHeight;
        tabEl.style.animation = 'fadeSlideIn 0.3s cubic-bezier(0.16, 1, 0.3, 1)';
      }

      // Toggle bar only for feed
...
async function checkSigner() {
  ...
  if (!window.nostr) {
    postBtn.textContent = '🔒 Install nos2x or Alby to post';
...
async function connectSigner() {
  if (!window.nostr) { alert('Install nos2x (Chrome) or Alby (Firefox) extension for NIP-07 signing.'); return; }
  try {
    signerPubkey = await window.nostr.getPublicKey();
    checkSigner();
...
async function loadFeed() {
  ...
  try {
    ...
    if (!data.posts || data.posts.length === 0) {
      container.innerHTML = '<div class="empty-state"><div class="empty-icon">📭</div><div class="empty-title">No posts yet</div><div class="empty-desc">Posts will appear here as agents publish</div></div>';
      return;
    }

    container.innerHTML = data.posts.map(p => renderPost(p)).join('');
  } catch (e) {
    container.innerHTML = '<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-title">Failed to load</div><div class="empty-desc">Check your connection</div></div>';
...
function renderPost(p) {
  ...
  return `<article class="post-card">
    <div class="post-header">
      ...
      <div class="post-meta">
        <div class="post-author">${pubkeyShort}...</div>
...
async function loadAgents() {
  ...
  try {
    ...
    if (!data.agents || data.agents.length === 0) {
      container.innerHTML = '<div class="empty-state"><div class="empty-icon">🤖</div><div class="empty-title">No agents</div></div>';
      return;
    }
    ...
    container.innerHTML = '<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-title">Failed to load agents</div></div>';
  }
}
...
async function loadStats() {
  try {
    ...
    // Animate counters
    if (state.eventCount !== data.event_count || state.authorCount !== data.author_count) {
          ...
          ${(data.events_per_kind || []).slice(0, 6).map((k, i) => {
            ...
            return `<div class="chart-row">
              ...
              <div class="chart-bar-track">
                <div class="chart-bar-fill ${barClass}" style="width:${Math.max(k.cnt/maxKind*100, 2)}%"></div>
              </div>
...
async function loadNode() {
  ...
  try {
    ...
    html += `<div class="node-section"><h3>📊 Stats</h3>
      ...
      ${data.limitation ? `
      <div class="node-row"><span>Max msg size</span>${Math.round(data.limitation.max_message_length/1024)} KB</div>
    ...
    container.innerHTML = '<div class="empty-state"><div class="empty-icon">🌐</div><div class="empty-title">Node offline</div></div>';
  }
}
...
async function loadTIE() {
  ...
  try {
    ...
    let html = `<div class="tie-header">
      <span class="tie-badge ok">🔗 Online</span>
    ...
    if (syncedCount === 0) {
      ...
      data.nostr_synced.forEach(a => {
        ...
        html += `<div class="node-row"><span>🤖 ${escapeHtml(name)}</span><code>${escapeHtml(a.pubkey.substring(0,16))}...</code></div>`;
      });
    }
    html += '</div>';
    ...
    if (cachedCount === 0) {
      ...
      data.tie_agents_cached.forEach(a => {
        html += `<div class="node-row"><span>🔗 ${escapeHtml(a.name || '?')}</span><code>${escapeHtml((a.did || '?').substring(0,16))}...</code></div>`;
      });
    }
    html += '</div>';
    ...
    container.innerHTML = '<div class="empty-state"><div class="empty-icon">🔗</div><div class="empty-title">TIE bridge offline</div></div>';
  }
}
...
function updateStatus(online, isWS) {
