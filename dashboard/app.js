...
function switchTab(name, push = true) {
  ...
  if (name === "audit") ensureAuditFrame();
}

/* ── аудит: ленивая вкладка, iframe грузится только когда её открыли ── */
const AUDIT_URL = new URL("audit/", location.href).href;
function ensureAuditFrame(scroll) {
  const box = $("auditFrameBox"), f = $("auditFrame");
  if (!box || !f) return;
  if (!f.getAttribute("src")) f.setAttribute("src", AUDIT_URL);
...
/* ── аудит: кнопки в шапке и внутри вкладки, плюс копирование наружной ссылки ── */
{
  const nav = $("navAudit");
  if (nav) nav.addEventListener("click", () => { switchTab("audit"); ensureAuditFrame(true); });
  const here = $("auditOpenHere");
  if (here) here.addEventListener("click", () => ensureAuditFrame(true));
  const field = $("auditUrl");
  if (field) field.value = AUDIT_URL;
  const HINT = "на странице проверки нет ни дашборда, ни наших данных — ссылку можно давать наружу";
  async function copyAudit() {
    const state = $("auditCopyState");
    let done = false;
    try { await navigator.clipboard.writeText(AUDIT_URL); done = true; } catch (e) {
      try { field.select(); done = document.execCommand("copy"); } catch (e2) { done = false; }
    }
    if (state) {
      state.textContent = done ? "скопировано: " + AUDIT_URL : "не удалось скопировать автоматически — выдели ссылку выше";
      setTimeout(() => { if (state) state.textContent = HINT; }, 2800);
    }
  }
  const c1 = $("auditCopy"), c2 = $("navAuditCopy");
  if (c1) c1.addEventListener("click", copyAudit);
  if (c2) c2.addEventListener("click", copyAudit);
}
...
window.addEventListener("hashchange", () => {
  ...
  if (["overview", "chain", "agents", "economy", "journal", "audit"].includes(h)) switchTab(h, false);
});
{ const h = location.hash.slice(1); if (["chain", "agents", "economy", "journal", "audit"].includes(h)) switchTab(h, false); }
