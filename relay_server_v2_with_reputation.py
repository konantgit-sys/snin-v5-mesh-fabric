...
# ═══ Phase 3: Reputation Gate ═══
try:
    from reputation_gate import get_reputation, is_reputation_trusted, get_reputation_stats, save_cache, REP_THRESHOLD, REP_MIN_READ
    REPUTATION_AVAILABLE = True
except ImportError as e:
    REPUTATION_AVAILABLE = False
    get_reputation = None
    is_reputation_trusted = None
    get_reputation_stats = None
    save_cache = None
    REP_THRESHOLD = 0.5
    REP_MIN_READ = 0.2
    logging.getLogger("snin").warning(f"Reputation Gate not available: {e}")
...
class NostrWSHandler:
    ...
    async def _handle_event(self, ws, event: dict, authed_pubkey: str | None, ws_id: str = ""):
        ...
        if remote_ip in ("127.0.0.1", "::1", "localhost"):
            ...
            pass
        # ═══ Phase 3: Reputation-aware access (Whitelist + Reputation Gate) ═══
        rep_trusted = False
        if REPUTATION_AVAILABLE:
            try:
                rep = get_reputation(pubkey)
                rep_trusted = rep.get("access") == "full"
            except Exception:
                rep_trusted = False
        
        if effective_pubkey not in WHITELIST and pubkey not in WHITELIST and not rep_trusted:
            ...
            if kind not in PUBLIC_WRITE_KINDS:
                # Check if reputation allows at least write access
                rep_write = False
                if REPUTATION_AVAILABLE:
                    try:
                        rep2 = get_reputation(effective_pubkey)
                        rep_write = rep2.get("access") in ("write", "full")
                    except Exception:
                        rep_write = False
                if not rep_write:
                    await ws.send_json(["OK", event_id, False, f"blocked: kind {kind} requires whitelist or reputation access"])
                    return
            logger.info(f"NIP-42: authed user {pubkey[:16]}... writing kind {kind} (reputation trusted: {rep_trusted})")
...
async def admin_stats(request):
    ...
    data = {
        **stats,
        ...
        "reputation": get_reputation_stats() if REPUTATION_AVAILABLE else {"enabled": False},
...
async def main():
    ...
    # ═══ Phase 3: Reputation cache periodic save ═══
    async def _rep_cache_saver():
        while True:
            await asyncio.sleep(300)  # каждые 5 минут
            if REPUTATION_AVAILABLE and save_cache:
                try:
                    save_cache()
                except Exception:
                    pass
    asyncio.create_task(_rep_cache_saver())
    logger.info("Reputation Gate cache save loop started (каждые 5 мин)")
